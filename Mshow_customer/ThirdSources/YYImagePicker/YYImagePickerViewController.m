...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, WIN_WIDTH, WIN_HEIGHT - BottomProductHeight - iPhone_ProtraitDisableViewHeight) collectionViewLayout:flowLayout];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _maxImagesCount = 3;
    [self setUpLeftItem:@"" selectedTitle:@"" image:@"img_arrow_left" highImage:@"img_arrow_left" target:self action:@selector(back)];
...
- (void)createUI
{
    ...
    if (self.imagesCount) {
        ...
        [button setTitle:[NSString stringWithFormat:@"完成(%@/%@)", @(self.imagesCount), @(_maxImagesCount)] forState:UIControlStateNormal];
    } else {
...
{
    if (indexPath.item == 0) {
        self.imagesCount + self.selectedAssetsArray.count >= _maxImagesCount ? [self showMessage] : [self openCamera];
    } else {
        YYBigImageViewController *bigImageVC = [[YYBigImageViewController alloc] init];
        bigImageVC.maxImagesCount = self.maxImagesCount;
...
- (void)showMessage
{
    ...
    hud.labelText = [NSString stringWithFormat:@"最多选择%@张图片", @(_maxImagesCount)];
...
- (void)didSelectImage:(NSIndexPath *)indexPath
{
    ...
    if (self.imagesCount + self.selectedAssetsArray.count >= _maxImagesCount) {
        [self showMessage];
        cell.isSelected = NO;
        return;
    }
...
- (void)updateSendButton
{
    if (self.selectedAssetsArray.count == 0) {
        ...
        [self.sendButton setTitle:[NSString stringWithFormat:@"完成(%@/%@)", @(imageCounts), @(_maxImagesCount)] forState:UIControlStateNormal];
...
{
    if (self.selectedAssetsArray.count + self.imagesCount >= _maxImagesCount) {
        return NO;
    }
    YYImagePickerCell *cell = (YYImagePickerCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index + 1 inSection:0]];
...
- (void)didReceiveMemoryWarning {
    ...
    [YYMemoryCacheHandle clearMemoryWarn];
}
