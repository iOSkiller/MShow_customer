...
- (UIButton *)editorButton {
    if (!_editorButton) {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(0, 0, RELATIVE_WIDTH(88), RELATIVE_WIDTH(88));
        [button setTitle:@"编辑" forState:UIControlStateNormal];
        [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [button addTarget:self action:@selector(editorAction) forControlEvents:UIControlEventTouchUpInside];
        button.titleLabel.font = [UIFont systemFontOfSize:RELATIVE_WIDTH(36)];
        _editorButton = button;
    }
...
- (DNFullImageButton *)fullImageButton {
    if (!_fullImageButton) {
        _fullImageButton = [[DNFullImageButton alloc] init];
...
- (void)updateSelestedNumber
{
    ...
    if (selectedNumber) {
        ...
        [self.sendButton setTitle:[NSString stringWithFormat:@"完成(%@/%@)",@(selectedNumber), @(_maxImagesCount)] forState:UIControlStateNormal];
...
#pragma mark - ImageEditor

- (void)editorAction
{
    if (_previousEditedImage != nil) {
        [self showImageEditor:_previousEditedImage index:self.currentIndex];
    }
...
- (void)showImageEditor:(UIImage *)image index:(NSInteger)index
{
    _previousNavigationBar = self.navigationController.navigationBar;
    self.collectionView.hidden = YES;
}

- (void)updateSelectedImages:(NSInteger)index image:(UIImage *)image
{
